...
public static class TlbxDirectory
{
    ...
    private const string ClaudeMdContent =
        ...
        **Rules:**
        ...
        Coding agents should keep the session topic aligned with the user's current high-level work area. Use `mt_topic` with a concise 3-6 word topic such as `DAI teacher demo cleanup` or `tlbx sidebar Git context`. Update it when the user shifts to a different work area, but not for every small subplot.
    ...
    private const string AgentsMdContent =
        ...
        mt_publish_status working "Implementing deterministic history" "Control plane API" "Run focused tests" tlbx
        mt_work_add mail "Answer rollout question" "Customer asked for the date" "Draft reply for approval" high JPA gmail-thread-42
        mt_checkpoint verified "Focused tests pass" "18 backend tests" tlbx
        ...
        `mt_repo` bindings are session-scoped and ad hoc. Coding agents should add every additional repo they use, inspect, or edit when that repo is not the session working directory. This keeps tlbx showing both repo states side by side in the IDE bar and a compact extra repo line under the sidebar cwd. The cwd repo remains automatic; `mt_repo remove REPO_ROOT` only removes extra bindings.
